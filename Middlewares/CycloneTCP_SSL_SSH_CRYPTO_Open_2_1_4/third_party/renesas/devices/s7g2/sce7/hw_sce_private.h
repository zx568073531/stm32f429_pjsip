...
extern ssp_err_t HW_SCE_Initialization1(void);

extern ssp_err_t HW_SCE_Initialization2(void);
...
ssp_err_t HW_SCE_secureBoot(void);

void                 HW_SCE_EndianSetBig(void);
void                 HW_SCE_EndianSetLittle(void);
crypto_word_endian_t HW_SCE_EndianFlagGet(void);
ssp_err_t            HW_SCE_McuSpecificInit(void);

ssp_err_t HW_SCE_FW_IntegrityChk(void);
ssp_err_t HW_SCE_HUK_Load(uint32_t * InData_LC);
ssp_err_t HW_SCE_HUK_Load_LCS(void);
ssp_err_t HW_SCE_p07(uint32_t * OutData_KeyIndex);
...
#endif
ssp_err_t HW_SCE_GenerateOemKeyIndexPrivate(const sce_oem_key_type_t key_type,
