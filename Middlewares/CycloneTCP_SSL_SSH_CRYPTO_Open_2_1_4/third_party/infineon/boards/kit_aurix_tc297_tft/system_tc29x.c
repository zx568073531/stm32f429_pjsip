...
* Function: Extended system control API implementation for TC29x
...
# define DEFAULT_PLL_VALUE		PLL_VALUE_300_100
#endif
...
static const PllInitValue_t g_PllInitValue_300_100 =
{
	...
	0x0007001C, 0x01017600, 0x00020505, 0x13130113, 0x13113343, 0x00000002, 1
};
...
{
	/* OSCCON,	PLLCON0,	PLLCON1,	CCUCON0,	CCUCON1,	CCUCON2,    finalK */
	0x0007001C, 0x01017600, 0x00020505, 0x12120112, 0x12112242, 0x00000002, 2
};
...
static void system_set_pll(const PllInitValue_t *pPllInitValue)
{
	...
	{
		unlock_wdtcon();
		FLASH0_FCON.B.WSECPF = (3 - 1);		/* 3 WS for ECC */
		FLASH0_FCON.B.WSPFLASH = (9 - 1);	/* 9 WS for Flash */
		lock_wdtcon();
	}
	else if (pPllInitValue == PLL_VALUE_200_100)
	{
		unlock_wdtcon();
		FLASH0_FCON.B.WSECPF = (2 - 1);		/* 2 WS for ECC */
		FLASH0_FCON.B.WSPFLASH = (6 - 1);	/* 6 WS for Flash */
		lock_wdtcon();
	}
